...
let make_handle (h : handle) : value = VHandle h
...
let rec string_of_value (v : value) : string =
  ...
  | VList _ -> "<list>"
  | VRef _ -> "<ref>"
(* | VList lst -> "[" ^ (lst |> List.map string_of_value |>
...
let unbound_err = "Unbound Variable"
let if_guard_err = "Guard of If Must be Bool"
let seq_err = "Sequence May Not Have More than 1 Non-Unit Value"
let pat_err = "Let Binding Pattern Mismatch"

let rec bind_pattern (p : pat) (v : value) : env option =
  match (p, v) with
  | Empty, VList x -> if x = [] then Some [] else None
  | WildCard, _ -> Some []
  | PVar x, _ -> Some [ (x, v) ]
  ...
  | PUnit a, VUnit b -> if a = b then Some [] else None
  | PCons (a, b), VList lst ->
      if lst <> [] then
        let h = List.hd lst in
        let t = List.tl lst in
        bind_pairs (a, b) (h, VList t)
      else None
  | PPair (a, b), VPair (c, d) -> bind_pairs (a, b) (c, d)
  | _ -> None

and bind_pairs (a, b) (c, d) =
  let x = bind_pattern a c in
  let y = bind_pattern b d in
  match (x, y) with Some e1, Some e2 -> Some (e1 @ e2) | _ -> None
...
let rec eval_expr (env : env) (expr : expr) : value =
  ...
  | EList Empty -> VList []
  | List (v, EList Empty) -> VList [ eval_expr env v ]
  | List (v, lst) -> VList [ eval_expr env v; eval_expr env lst ]
  | EList _ -> failwith "Syntax Error"
  | Seq (e1, e2) ->
      eval_expr env e1;
      eval_expr env e2
  | Match (e, cs) -> eval_match env e cs
...
and eval_bop env bop e1 e2 =
  ...
  | Cons, v, VList lst -> VList (v :: lst)
  | Assign, VRef r, v -> VUnit (r := v)
  | _ -> failwith bop_err

and eval_uop env bop e1 =
  match (bop, eval_expr env e1) with
  | Neg, VInt n -> VInt (-n)
...
and eval_let env x e1 e2 =
  ...
  let env' =
    ...
    | Some e -> prepend_env e env
    | None -> raise (TypeError "Let Binding Pattern Mismatch")
  in
  eval_expr env' e2

and eval_var env x =
  try find_env env x with Not_found -> failwith unbound_err
...
and eval_app env e1 e2 =
  match assert_function (eval_expr env e1) with
  | p, e, env -> eval_let env p e2 e

and eval_match env e cs =
  match cs with
  | h :: t ->
      if match_helper env e h <> VString "No Pattern" then
        match_helper env e h
      else eval_match env e t
  | [] -> raise InexhaustivePatterns

and match_helper env e1 (x, y) =
  match eval_let env x e1 y with
  | exception TypeError _ -> VString "No Pattern"
  | a -> a

let eval_defn (env : env) (defn : defn) : env =
  match defn with
  | Def (p, v) -> (
      match bind_pattern p (eval_expr env v) with
      | Some e -> prepend_env e env
      | None -> raise (TypeError "Let Binding Pattern Mismatch"))
