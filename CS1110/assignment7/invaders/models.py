...
class Ship(GSprite):
    ...
    def __init__(self, format, x, y, width, height, source):
        """
        Initializes the player spaceship at coordinates (x,y).
        ...
        Parameter source: the png file name that the image for the ship is stored
        Precondition: source a string that represents the file name for the image (ends in .png)
        """
        super().__init__(format=format, x=x, y=y, width=width,
    ...
    def scollides(self, bolt):
        ...
        This methods returns False if bolt was not fired by the player
        ...
        if bolt._velocity != -BOLT_SPEED:
            return False
        topl = (bolt.x-BOLT_WIDTH*0.5, bolt.y+BOLT_HEIGHT*0.5)
        topr = (bolt.x+BOLT_WIDTH*0.5, bolt.y+BOLT_HEIGHT*0.5)
        bottoml = (bolt.x-BOLT_WIDTH*0.5, bolt.y-BOLT_HEIGHT*0.5)
        bottomr = (bolt.x+BOLT_WIDTH*0.5, bolt.y-BOLT_HEIGHT*0.5)
        s = self
        if (s is not None and (s.contains(topl)or s.contains(topr) or
        s.contains(bottoml)or s.contains(bottomr))):
            return True
        return False
...
class Alien(GImage):
    ...
    def __init__(self, x, y, width, height, source):
        """
        Initializes the individual aliens in the wave at the coordinate (x,y).
        ...
        Parameter source: the png file name that the image for the alien is stored
        Precondition: source a string that represents the file name for the image (ends in .png)
        """
    ...
    def acollides(self, bolt):
        ...
        bottomr = (bolt.x+BOLT_WIDTH*0.5, bolt.y-BOLT_HEIGHT*0.5)
        a = self
        if (a is not None and (a.contains(topl)or a.contains(topr)or
        a.contains(bottoml)or a.contains(bottomr))):
            return True
        return False
