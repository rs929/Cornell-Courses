"""
A module to show off how to use a subcontroller to pause
an animation.

As we can see in the state.py module, combining animation
with key management can be quite complicated.  It is often
easier to handle animation in a subcontroller and turn it
off in the main app.

The subcontroller should always be a subclass of object.
Instances of GameApp create a window to draw in, and we
only want one window for drawing.
...
class MainApp(GameApp):
    """
    This class represents the main application with a
    drawing window.

    This application listens for touches, pausing and
    restarting the animation every time there is a click.
    ...
    NOTE: The additional attributes are hidden in this
    example, because we have more than one class. The rule is that, when you have more than one class, the
    ...
    def _checkPaused(self):
        ...
        The rules for pressing a key are the same as state.py.
        A key press is when a key is pressed for the FIRST
        TIME. We do not want the animation to pause and unpause
        while we hold down the key. The user must release
...
class Animation(object):
    """
    This class is a subcontroller to animate an ellipse
    about a center.
    ...
    NOTE: Attributes are hidden in this example, because
    we have more than one class.  The rule is that, when
    you have more than one class, the one class cannot
    access the hidden attributes or methods of another.
    """
    # HIDDEN ATTRIBUTES
    ...
    #
    # Attribute _angle: ellipse angle from center
    # Invariant: _angle is a float
    #
    # Attribute _ellipse: the ellipse to animate
    # Invariant: _ellipse is a GEllipse whose center is (RADIUS,self.angle) in polar
...
if __name__ == '__main__':
    MainApp(left=150,width=WINDOW_WIDTH,height=WINDOW_HEIGHT,fps=60.0).run()
